...
let changeHospitalDetails = context(() => {
    intent('Bed count is $(NUMBER)', p=> {
       ...
       projectAPI.checkHospitalPage = function(p, param, callback) {
        if (`${param.value}`!=="https://versatilevats.tech/hackfest/profile.php?action=hospital") {
...
intent(noctx,'Why HelloSign is used in this project', p=> {
   p.play('To enable the e-sign functionality and take part in their event. Most probably win that');
});
...
let addName = context(() => {
     intent('Name is $(NAME)', p=> {
       ...
       projectAPI.checkPatientPage = function(p, param, callback) {
        if (`${param.value}`!=="https://versatilevats.tech/hackfest/profile.php?action=create_user") {
...
intent('Patients referred to my hospital', p=> {
   p.play({command: 'referredToUs'});
});

projectAPI.referToUs = function(p, param, callback) {
  p.play(`${param.value}`);
};

projectAPI.noRecord = function(p, param, callback) {
  p.play(`${param.value}`);
};

intent('(Please) Show details of patient $(NUMBER) ',p=> {
    p.play(`Here are the details of patient no ${p.NUMBER.value}`);
    p.play({command:'getDetails', item: p.NUMBER.value});
});

intent('(Please) Add reports of patient $(NUMBER) ',p=> {
    p.play(`Taking you there`);
    p.play({command:'uploadReports', item: p.NUMBER.value});
});

intent('(Please) delete patient $(NUMBER) ',
       '(Remove|delete) patient $(NUMBER)',p=> {
        p.play(`Successfully Deleted`);
        p.play({command:'deletePatient', item: p.NUMBER.value});
});
