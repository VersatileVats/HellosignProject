...
<body id="page-top" style="user-select: none">
        ...
        <div class="d-flex flex-column" id="content-wrapper">
            <div id="content">
                            ...
                            <li class="nav-item dropdown no-arrow">
                                <div class="nav-item dropdown no-arrow"><a class="dropdown-toggle nav-link" aria-expanded="false" data-bs-toggle="dropdown" href="#"><span class="d-none d-lg-inline me-2 text-gray-600 small"><?php echo($_SESSION['name']) ?></span><img class="border rounded-circle img-profile" src="https://img.icons8.com/ios-glyphs/30/000000/test-account.png"/></a>
                                    <div class="dropdown-menu shadow dropdown-menu-end animated--grow-in">
                                        <?php $directory = 'utilities/helloworks/pAgreements/'.$_SESSION['id'].'.pdf';   ?>
                                        <a class="dropdown-item"target="_blank" href=<?php echo $directory ?>><i class="fas fa-sign-out-alt fa-sm fa-fw me-2 text-gray-400"></i>&nbsp;View Signed Agreement</a>
                ...
                <div class="container-fluid">
                    ...
                    <div class="row mb-3">
                        <div class="col">
                            <h3 class="text-dark mb-4" style="text-align: center">Real time stats</h3>
                            
                            <div class="row">
                                <div class="col">
                                    <div class="card shadow mb-3">
                                        <div class="card-header py-2">
                                            <p class="text-primary m-0 fw-bold">Get to kknow real time stats of the registered hospitals</p>
                                        </div>
                                        <div class="card-body">
                                            <form action="utilities/refer_patients.php?p_id=<?php echo $p_id ?>" method="post">
                                                
                                                <div class="row">
                                                    <div class="col text-center" id="sort">
                                                        <div class="mb-3 form-check form-check-inline d-sm-block d-md-inline-block">
                                                            <input type="radio" class="form-check-input" name="get_real" value="beds">
                                                           <label class="form-label">Beds</label>
                                                        </div>
                                                        <div class="mb-3 form-check form-check-inline d-sm-block d-md-inline-block">
                                                            <input type="radio" class="form-check-input" name="get_real" value="doctor">
                                                           <label class="form-label">Doctor</label>
                                                        </div>
                                                        <div class="mb-3 form-check form-check-inline">
                                                            <input type="radio" class="form-check-input" name="get_real" value="icu">
                                                           <label class="form-label">ICU</label>
                                                        </div>
                                                        <div class="mb-3 form-check form-check-inline">
                                                            <input type="radio" class="form-check-input" name="get_real" value="oxygen">
                                                           <label class="form-label">Oxygen</label>
                                                        </div>
                                                    </div>
                                                </div>
                                                
                                                <div class="row" id="noRealStats" style="display: none">
                                                    <div class="col">
                                                        <div class="mb-3 text-center text-danger" style="font-weight: bold">
                                                            <p>No hospitals are registered on the platfrom yet. </p>
                                                        </div>
                                                    </div>
                                                </div>
                                                
                                    <div class="table-responsive table mt-2" id="options_realStats" role="grid" aria-describedby="dataTable_info">
                                        <table class="table my-0">
                                            <thead>
                                                <tr>
                                        ...
                                        <table class="table my-0" id="real_stats">
    ...
    <script>
        ...
        document.getElementById('sort').addEventListener("click", async () => {
            var radios = document.getElementsByName('get_real');
            for (var radio of radios) {
                if(radio.checked) {
                    document.getElementById('real_stats').innerHTML = "";
                    
                    let xhr  = new XMLHttpRequest();
                    xhr.open("POST","real_stats.php");
                    xhr.setRequestHeader("Content-type","application/x-www-form-urlencoded");
                    xhr.send(`real_time=${radio.value}`);
                    xhr.onload = function () {
                        let arr = this.responseText.split("&");
                        if(arr == "NA") {
                            document.getElementById('noRealStats').style.display="block";
                            document.getElementById('options_realStats').style.disaply = "none";
                        } else {
                            document.getElementById('noRealStats').style.display="none";
                            document.getElementById('options_realStats').style.disaply = "block";
                            res =  arr.slice(0,length - 1);
                            res.forEach(updateUI); 
                        }
                        ...
                        function updateUI(item) {
                            ...
                            var newHTML = '<tbody><td>'+item.split("*")[0]+'</td><td>'+item.split("*")[1]+'</td><td>'+item.split("*")[3]+'</td><td>'+item.split("*")[4]+'</td><td>'+item.split("*")[5]+'</td><td>'+item.split("*")[6]+'</td></tbody>';
                            
                            document.getElementById('real_stats').innerHTML+=newHTML; 
                        }
                    }
                }
            }
